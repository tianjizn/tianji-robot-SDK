...
extern "C"
{
    ...
    */
    ...
    /**
     * @brief Initialize the Luna body kinematics model.
     *
     * @param[in] handle Kinematics context handle.
     * @param[in] DH Modified Denavit-Hartenberg parameters of the Luna body,
     *               represented as a 6-by-4 matrix. Each row contains
     *               `[alpha, a, d, theta_offset]` for one joint.
     * @param[in] Flange_length Flange length along the flange-frame Z-axis.
     * @param[in] lmt_neg Lower joint limits, length 6.
     * @param[in] lmt_pos Upper joint limits, length 6.
     * @return `FX_MOTION_OK` on success, otherwise `FX_MOTION_ERROR`.
     */
    KINEMATICS_SDK_API int FX_L0_Kinematics_luna_body_init(
        FX_MotionHandle handle,
        double DH[6][4], 
        double Flange_length, 
        double lmt_neg[6], 
        double lmt_pos[6]);    

    /**
     * @brief Compute the left and right shoulder poses using forward kinematics.
     *
     * @param[in] handle Kinematics context handle.
     * @param[in] jv Body joint values, length 6.
     * @param[out] pgL Left body pose represented as a 4-by-4 homogeneous
     *                 transformation matrix.
     * @param[out] pgR Right body pose represented as a 4-by-4 homogeneous
     *                 transformation matrix.
     * @return `FX_MOTION_OK` on success, otherwise `FX_MOTION_ERROR`.
     */
    KINEMATICS_SDK_API int FX_L0_Kinematics_luna_body_forward_kinematics(
        FX_MotionHandle handle,
        double jv[6], 
        double pgL[4][4], 
        double pgR[4][4]);
    ...
    /**
     * @brief Compute luna body joint values from the left and right shoulder poses.
     *
     * @param[in] handle Kinematics context handle.
     * @param[in] pgL Left shoulder pose represented as a 4-by-4 homogeneous
     *                transformation matrix.
     * @param[in] pgR Right shoulder pose represented as a 4-by-4 homogeneous
     *                transformation matrix.
     * @param[in] ref_joints Reference body joint values used to select an
     *                       inverse kinematics solution, length 6.
     * @param[out] ret_joints Computed body joint values, length 6.
     * @return `FX_MOTION_OK` on success, otherwise `FX_MOTION_ERROR`.
     */
    KINEMATICS_SDK_API int FX_L0_Kinematics_luna_body_inverse_kinematics(
        FX_MotionHandle handle,
        double pgL[4][4], 
        double pgR[4][4],
        double ref_joints[6],
